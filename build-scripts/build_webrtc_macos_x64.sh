...
./build.sh -a $ARTOOL -q -o $WEBRTC_DIR -b $WEBRTC_BRANCH -t mac -c x64 -e 1 -z "use_custom_libcxx_for_host=false rtc_enable_protobuf=false use_custom_libcxx=false" $NOFETCH
rc=$?
